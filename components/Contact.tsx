...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!goal) {
      setErrorMessage("Please select a primary goal.");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/public/coaching-applications`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <section className="py-32 px-margin-mobile md:px-margin-desktop bg-surface relative border-t border-white/5 overflow-hidden" id="contact">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        {/* Right Column: Brutalist Form / Success Card */}
        <div className="lg:col-span-7">
          {isSubmitted ? (
            <div className="bg-surface-container border border-black/10 dark:border-white/10 p-8 md:p-12 shadow-2xl relative text-center min-h-[480px] flex flex-col items-center justify-center">
              {/* Form Corner Accents */}
