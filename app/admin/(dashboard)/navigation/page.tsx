...
export default function NavigationPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/v1/admin/navigation");
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.id) {
        await api.put(`/api/v1/admin/navigation/${formData.id}`, formData);
        showSuccess("Link updated successfully.");
      } else {
        await api.post("/api/v1/admin/navigation", formData);
      ...
      <div className="p-8 max-w-2xl mx-auto space-y-8">
        ...
        <form onSubmit={handleSubmit} className="bg-surface p-8 rounded-2xl border border-white/5 space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="openInNewTab" className="text-on-surface font-label-caps uppercase tracking-widest text-sm select-none">Open In New Tab</label>
          </div>
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-display text-3xl uppercase tracking-wider text-on-surface">Navigation Links</h1>
          <p className="text-on-surface-variant font-body-md text-sm mt-1">Manage main navbar navigation links.</p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-on-surface-variant">Loading links...</div>
      ) : (
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                  <td className="p-4 text-on-surface font-semibold">{item.label}</td>
                  <td className="p-4 text-sm text-on-surface-variant">{item.href}</td>
                  ...
                  <td className="p-4 flex gap-2 justify-end">
                    ...
                    <Button variant="ghost" className="p-2 min-h-0 text-error hover:bg-error/10" onClick={() => handleDelete(item.id, item.label)}>
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-on-surface-variant italic">No links found.</td>
                </tr>
              )}
            </tbody>
          </table>
