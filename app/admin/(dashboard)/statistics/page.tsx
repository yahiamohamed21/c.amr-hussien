...
export default function StatisticsPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/v1/admin/statistics");
      setItems(res.data);
    } catch (err) {
  ...
  const handleDelete = async (id: string, label: string) => {
    if (await confirmDelete(label)) {
      try {
        await api.delete(`/api/v1/admin/statistics/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.id) {
        await api.put(`/api/v1/admin/statistics/${formData.id}`, formData);
        showSuccess("Statistic updated successfully.");
      } else {
        await api.post("/api/v1/admin/statistics", formData);
    ...
    return (
      <div className="p-8 max-w-2xl mx-auto space-y-8">
        ...
        <form onSubmit={handleSubmit} className="bg-surface p-8 rounded-2xl border border-white/5 space-y-6">
          ...
          />
          <Input
            label="Label (e.g. Active Clients, Success Rate)"
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="font-display text-3xl uppercase tracking-wider text-on-surface">Statistics</h1>
          <p className="text-on-surface-variant font-body-md text-sm mt-1">Manage stats counters displayed on the homepage.</p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-on-surface-variant">Loading statistics...</div>
      ) : (
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                  <td className="p-4 text-xl font-display text-primary">
                    {item.value}
                    {item.suffix && <span className="text-sm font-bold text-on-surface">{item.suffix}</span>}
                  </td>
                  <td className="p-4 text-on-surface">{item.label}</td>
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-on-surface-variant italic">No statistics found.</td>
                </tr>
              )}
            </tbody>
          </table>
